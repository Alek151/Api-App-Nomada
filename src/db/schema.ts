...
export const posts = pgTable('posts', {
  id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), destinationId: uuid('destination_id').references(() => destinations.id, { onDelete: 'set null' }), caption: varchar('caption', { length: 800 }).notNull(), visibility: varchar('visibility', { length: 16 }).default('public').notNull(), status: varchar('status', { length: 16 }).default('published').notNull(), likeCount: integer('like_count').default(0).notNull(), commentCount: integer('comment_count').default(0).notNull(), ...audit,
