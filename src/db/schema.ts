...
export const destinations = pgTable('destinations', {
  id: uuid('id').primaryKey().defaultRandom(), slug: varchar('slug', { length: 120 }).notNull(), name: varchar('name', { length: 160 }).notNull(), country: varchar('country', { length: 100 }).default('Guatemala').notNull(), countryCode: varchar('country_code', { length: 2 }).default('GT').notNull(), officialName: varchar('official_name', { length: 200 }), municipality: varchar('municipality', { length: 100 }), department: varchar('department', { length: 100 }).notNull(), category: varchar('category', { length: 60 }).notNull(), subcategory: varchar('subcategory', { length: 100 }), description: text('description').notNull(), shortDescription: varchar('short_description', { length: 320 }), history: text('history'), historicalPeriod: varchar('historical_period', { length: 160 }), approximateDate: varchar('approximate_date', { length: 160 }), unescoStatus: boolean('unesco_status').default(false).notNull(), unescoType: varchar('unesco_type', { length: 40 }), websiteOfficial: text('website_official'), wikipediaUrl: text('wikipedia_url'), coordinatesSource: varchar('coordinates_source', { length: 160 }), historySource: varchar('history_source', { length: 240 }), latitude: doublePrecision('latitude').notNull(), longitude: doublePrecision('longitude').notNull(), validationRadiusMeters: integer('validation_radius_meters').default(100).notNull(), coverKey: text('cover_key'), activities: jsonb('activities').$type<string[]>().default([]).notNull(), visitDurationMinutes: integer('visit_duration_minutes'), schedule: jsonb('schedule').$type<Record<string, unknown>>().default({}), accessibility: jsonb('accessibility').$type<Record<string, unknown>>().default({}), visitorInfo: jsonb('visitor_info').$type<Record<string, unknown>>().default({}), nomadaRecommendations: jsonb('nomada_recommendations').$type<Record<string, unknown>>().default({}), nomadaCertifiedAt: timestamp('nomada_certified_at', { withTimezone: true }), averageCostMin: integer('average_cost_min'), averageCostMax: integer('average_cost_max'), costCurrency: varchar('cost_currency', { length: 3 }).default('GTQ').notNull(), contentStatus: varchar('content_status', { length: 24 }).default('published').notNull(), humanVerifiedAt: timestamp('human_verified_at', { withTimezone: true }), humanVerifiedBy: uuid('human_verified_by').references(() => users.id, { onDelete: 'set null' }), verificationNotes: text('verification_notes'), sourceUrl: text('source_url'), points: integer('points').default(100).notNull(), isActive: boolean('is_active').default(true).notNull(), metadata: jsonb('metadata').$type<Record<string, unknown>>().default({}), ...audit,
}, (t) => [uniqueIndex('destinations_slug_uq').on(t.slug), index('destinations_department_idx').on(t.department), index('destinations_country_idx').on(t.countryCode), index('destinations_admin_catalog_idx').on(t.countryCode, t.name)]);
...
/** Destinos guardados por el viajero, organizables en listas personales. */
export const savedDestinations = pgTable('saved_destinations', { destinationId: uuid('destination_id').notNull().references(() => destinations.id, { onDelete: 'cascade' }), userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), collection: varchar('collection', { length: 80 }).default('Guardados').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [primaryKey({ columns: [t.destinationId, t.userId] }), index('saved_destinations_user_idx').on(t.userId, t.collection, t.createdAt)]);
...
/** Eventos in-app; el dispositivo puede después registrar un token para push sin perder el historial. */
export const notifications = pgTable('notifications', { id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), type: varchar('type', { length: 40 }).notNull(), title: varchar('title', { length: 160 }).notNull(), body: varchar('body', { length: 360 }).notNull(), data: jsonb('data').$type<Record<string, unknown>>().default({}).notNull(), readAt: timestamp('read_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [index('notifications_user_created_idx').on(t.userId, t.createdAt)]);

/** Reportes y bloqueos para que la comunidad pueda moderarse de forma trazable. */
export const contentReports = pgTable('content_reports', { id: uuid('id').primaryKey().defaultRandom(), reporterUserId: uuid('reporter_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), resourceType: varchar('resource_type', { length: 32 }).notNull(), resourceId: uuid('resource_id').notNull(), reason: varchar('reason', { length: 80 }).notNull(), details: varchar('details', { length: 600 }), status: varchar('status', { length: 24 }).default('open').notNull(), reviewedBy: uuid('reviewed_by').references(() => users.id, { onDelete: 'set null' }), reviewedAt: timestamp('reviewed_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [index('content_reports_status_created_idx').on(t.status, t.createdAt)]);
export const userBlocks = pgTable('user_blocks', { blockerUserId: uuid('blocker_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), blockedUserId: uuid('blocked_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [primaryKey({ columns: [t.blockerUserId, t.blockedUserId] })]);

/** Una conexión requiere consentimiento de ambas partes antes de habilitar el chat. */
export const travelConnections = pgTable('travel_connections', { id: uuid('id').primaryKey().defaultRandom(), requesterUserId: uuid('requester_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), recipientUserId: uuid('recipient_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), introduction: varchar('introduction', { length: 500 }), status: varchar('status', { length: 24 }).default('pending').notNull(), respondedAt: timestamp('responded_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(), updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [uniqueIndex('travel_connections_pair_uq').on(t.requesterUserId, t.recipientUserId), index('travel_connections_recipient_idx').on(t.recipientUserId, t.status, t.createdAt), index('travel_connections_requester_idx').on(t.requesterUserId, t.status, t.createdAt)]);
export const directMessages = pgTable('direct_messages', { id: uuid('id').primaryKey().defaultRandom(), connectionId: uuid('connection_id').notNull().references(() => travelConnections.id, { onDelete: 'cascade' }), senderUserId: uuid('sender_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), body: varchar('body', { length: 1000 }).notNull(), readAt: timestamp('read_at', { withTimezone: true }), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [index('direct_messages_connection_created_idx').on(t.connectionId, t.createdAt)]);
export const profileLikes = pgTable('profile_likes', { profileUserId: uuid('profile_user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull() }, (t) => [primaryKey({ columns: [t.profileUserId, t.userId] }), index('profile_likes_profile_idx').on(t.profileUserId)]);
