...
app.get('/api/v1/me/travel-connections', requireAuth, async (c) => { const db = getDb(c.env); const userId = c.get('userId'); const connections = await db.select().from(travelConnections).where(sql`${travelConnections.requesterUserId} = ${userId} or ${travelConnections.recipientUserId} = ${userId}`).orderBy(desc(travelConnections.updatedAt)); const otherIds = connections.map((connection) => connection.requesterUserId === userId ? connection.recipientUserId : connection.requesterUserId); const people = otherIds.length ? await db.select({ user: users, profile: profiles }).from(users).innerJoin(profiles, eq(users.id, profiles.userId)).where(inArray(users.id, otherIds)) : []; const peopleById = new Map(people.map((person) => [person.user.id, publicUser(person.user, person.profile)])); return c.json({ data: connections.map((connection) => ({ ...connection, traveler: peopleById.get(connection.requesterUserId === userId ? connection.recipientUserId : connection.requesterUserId), incoming: connection.recipientUserId === userId })) }); });
