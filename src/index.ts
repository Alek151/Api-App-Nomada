...
app.post('/api/v1/posts', requireAuth, async (c) => { const parsed = z.object({ caption: z.string().min(1).max(800), destinationId: z.string().uuid().optional(), locationLabel: z.string().min(2).max(160).optional(), latitude: z.number().min(-90).max(90).optional(), longitude: z.number().min(-180).max(180).optional(), visibility: z.enum(['public', 'followers', 'private']).default('public'), mediaKeys: z.array(z.string().min(1)).min(1).max(10), mediaTypes: z.array(z.enum(['image', 'video'])).max(10).optional() }).superRefine((value, ctx) => { if ((value.latitude === undefined) !== (value.longitude === undefined)) ctx.addIssue({ code: 'custom', message: 'Las coordenadas deben enviarse juntas.', path: ['latitude'] }); }).safeParse(await c.req.json().catch(() => null)); if (!parsed.success) return c.json({ error: 'validation_error', fields: parsed.error.flatten().fieldErrors }, 422); const db = getDb(c.env); const post = await db.transaction(async (tx) => { const [created] = await tx.insert(posts).values({ userId: c.get('userId'), caption: parsed.data.caption, destinationId: parsed.data.destinationId, locationLabel: parsed.data.locationLabel, latitude: parsed.data.latitude, longitude: parsed.data.longitude, visibility: parsed.data.visibility }).returning(); await tx.insert(postMedia).values(parsed.data.mediaKeys.map((objectKey, position) => ({ postId: created.id, objectKey, mediaType: parsed.data.mediaTypes?.[position] ?? 'image', position }))); return created; }); return c.json({ post }, 201); });
